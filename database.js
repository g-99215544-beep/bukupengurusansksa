...
/**
 * DATA GURU (EDIT DI SINI)
 * Format:
 * { name: "Cikgu Abu", roles: ["Guru Kelas 5C", "Penyelaras HEM", "AJK APDM"] }
 */
const TEACHERS = [
  { name: "Cikgu Abu", roles: ["Guru Kelas", "Penyelaras HEM", "AJK APDM"] },
  { name: "Cikgu Aina", roles: ["Guru Kelas", "Setiausaha Panitia", "AJK Kokurikulum"] },
  { name: "Cikgu Farid", roles: ["Penyelaras ICT", "AJK Data", "Guru Bertugas"] },
];
...
/* ===== Feature: Profil Tugas Guru (tersorok dalam drawer tab "Guru") ===== */
...
function renderTeacherDetail(t){
  ...
  if (!t){
    els.teacherHint.textContent = "Tugas akan dipaparkan di sini.";
    els.teacherChips.innerHTML = "";
    return;
  }
  const roles = (t.roles || []).filter(Boolean);
  els.teacherHint.textContent = roles.length ? `Jumlah tugas: ${roles.length}` : "Tiada tugas direkodkan.";
  els.teacherChips.innerHTML = roles.map(r => `<span class="chip">${escapeHtml(r)}</span>`).join("");
}
...
function renderTeacherResults(list){
  ...
  if (!list.length){
    ...
    empty.textContent = "Tiada nama sepadan.";
  ...
  list.slice(0, 30).forEach(t => {
    ...
    const rolePreview = (t.roles || []).slice(0, 3).join(", ");
...
function initTeacherFeature(){
  const all = [...TEACHERS].sort((a,b)=> (a.name||"").localeCompare(b.name||"", "ms"));
  renderTeacherResults(all);
  renderTeacherDetail(null);

  els.teacherSearch.addEventListener("input", () => {
    const q = norm(els.teacherSearch.value);
    if (!q){ renderTeacherResults(all); return; }
    renderTeacherResults(all.filter(t => norm(t.name).includes(q)));
  });
}
/* ================================================================ */
